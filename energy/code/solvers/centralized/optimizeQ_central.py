...
class OptimizeQ_central(BaseSolver):
    """Employs a central solver optimizing Q and load consumption of controllables"""
    ...
    def __init__(self,solver_type, models, neighborhood, inputs_dict, price):
        ...
        """
        super(OptimizeQ_central,self).__init__(solver_type, models, neighborhood, inputs_dict, price)
        


    def _setup(self,inputs_dict):
        """Sets up hyperparameters as class attributes."""
        super(OptimizeQ_central,self)._setup(inputs_dict)
    ...
    def solve(self):
        ...
        for j in range(self.horizon):
            ...
            obj_term1=Q[j]+ self.Q_modify[j]-(quicksum(home_reals[i][t] for i in range(self.num_homes)for t in real_pow_idx_sel))
