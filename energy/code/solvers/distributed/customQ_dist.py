...
class CustomQ_dist(BaseSolver):
    """Employs a distributed solver without optimizing Q"""

    def __init__(self,solver_type, models, neighborhood, inputs_dict, price):
        """
        Initializes the distributed solver maintaining balance between supply and demand
        by providing additional power by powertobuy (without optimizing Q).
        
        Args:
            
        """
        super(CustomQ_dist,self).__init__(solver_type, models, neighborhood, inputs_dict, price)
        


    def _setup(self,inputs_dict):
        """Sets up hyperparameters as class attributes."""
        super(CustomQ_dist,self)._setup(inputs_dict)
    ...
    def _getQvalues(self, Q=None):
        """
        customQ_dist does not optimize Q, instead it considers powertobuy parameter
